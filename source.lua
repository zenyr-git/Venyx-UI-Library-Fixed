...
do
	function utility:Create(instance, properties, children)
		...
		for i, v in pairs(properties or {}) do
			...
			if typeof(v) == "Color3" then -- save for theme changer later
				...
				if theme then
					cache.objects[theme] = cache.objects[theme] or {}
					cache.objects[theme][i] = cache.objects[theme][i] or setmetatable({}, {_mode = "k"})
					
					table.insert(cache.objects[theme][i], object)
				end
			end
		end
	...
	function utility:Pop(object, shrink)
		...
		if not success then
			warn("Error in Pop animation: " .. err)
		end
	end
		...
		input.InputEnded:Connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				for i, callback in pairs(self.ended) do
					...
					if not success then
						warn("Dragging ended callback error: " .. err)
					end
				end
			end
		end)
...
do
	...
	function library.new(title)
		local container = utility:Create("ScreenGui", {
			...
			utility:Create("ImageLabel", {
				...
				ImageColor3 = cache.themes.Background,
				...
				utility:Create("ImageLabel", {
					...
					ImageColor3 = cache.themes.DarkContrast,
					ScaleType = Enum.ScaleType.Slice,
					SliceCenter = Rect.new(4, 4, 296, 296)
				}, {
				...
				utility:Create("ImageLabel", {
					...
					ImageColor3 = cache.themes.Accent,
					ScaleType = Enum.ScaleType.Slice,
					SliceCenter = Rect.new(4, 4, 296, 296)
				}, {
					utility:Create("TextLabel", { -- title
						...
						Text = title,
						TextColor3 = cache.themes.TextColor,
						TextSize = 14,
	...
	function page.new(library, title, icon)
		local button = utility:Create("TextButton", {
			...
			icon and utility:Create("ImageLabel", {
				...
				ImageColor3 = cache.themes.TextColor,
		...
		local container = utility:Create("ScrollingFrame", {
			...
			ScrollBarImageColor3 = cache.themes.DarkContrast,
			Visible = false
		}, {
			utility:Create("UIListLayout", {
				SortOrder = Enum.SortOrder.LayoutOrder,
	...
	function section.new(page, title)
		local container = utility:Create("ImageLabel", {
			...
			ImageColor3 = cache.themes.LightContrast,
			ScaleType = Enum.ScaleType.Slice,
			SliceCenter = Rect.new(4, 4, 296, 296),
			ClipsDescendants = true
		}, {
	...
	function library:Notify(title, text, callback)
		...
		notification.Accept.MouseButton1Click:Connect(function()
			...
			if callback then
				...
				if not success then
					warn("Notification accept callback error: " .. err)
				end
			end
		...
		notification.Decline.MouseButton1Click:Connect(function()
			...
			if callback then
				...
				if not success then
					warn("Notification decline callback error: " .. err)
				end
			end
	...
	function section:addButton(title, callback)
		...
		button.MouseButton1Click:Connect(function()
			...
			if callback then
				local success, err = pcall(callback, function(...)
	...
	function section:addToggle(title, default, callback)
		...
		toggle.MouseButton1Click:Connect(function()
			...
			if callback then
				local success, err = pcall(callback, active, function(...)
	...
	function section:addTextbox(title, default, callback)
		...
		input:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if callback then
				local success, err = pcall(callback, input.Text, nil, function(...)
		...
		input.FocusLost:Connect(function()
			...
			if callback then
				local success, err = pcall(callback, input.Text, true, function(...)
					self:updateTextbox(textbox, ...)
				end)
				if not success then
					warn("Textbox focus lost callback error: " .. err)
				end
			end
		end)
	...
	function section:addKeybind(title, default, callback, changedCallback)
		...
		self.binds[keybind] = {callback = function()
			...
			if callback then
				...
				if not success then
					warn("Keybind press callback error: " .. err)
				end
			end
		end}
		...
		keybind.MouseButton1Click:Connect(function()
			...
			if text.Text == "None" then -- new bind
				...
				if changedCallback then
					local success, err = pcall(changedCallback, key, function(...)
						self:updateKeybind(keybind, ...)
					end)
					if not success then
						warn("Keybind changed callback error: " .. err)
					end
				end
			end
		end)
	...
	function section:addColorPicker(title, default, callback)
		...
		local tab = utility:Create("ImageLabel", {
			...
			utility:Create("Frame", {
				...
				utility:Create("ImageButton", {
					Name = "Canvas",
					BackgroundTransparency = 1,
					BorderColor3 = cache.themes.LightContrast,
					...
					utility:Create("ImageLabel", {
						Name = "Cursor",
						BackgroundColor3 = cache.themes.TextColor,
						AnchorPoint = Vector2.new(0.5, 0.5),
				...
				utility:Create("Frame", {
					...
					utility:Create("ImageLabel", {
						...
						utility:Create("TextBox", {
							...
							ZIndex = 2,
							Font = Enum.Font.Gotham,
							PlaceholderColor3 = cache.themes.DarkContrast,
							Text = "255",
							TextColor3 = cache.themes.TextColor,
							TextSize = 10.000
						})
		...
		local callback_func = function(value)
			if callback then
				local success, err = pcall(callback, value, function(...)
	...
	function section:addDropdown(title, list, callback)
		...
		search.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
			if focused then
				local filtered_list = utility:Sort(search.TextBox.Text, list)
				filtered_list = #filtered_list ~= 0 and filtered_list or nil
				
				self:updateDropdown(dropdown, nil, filtered_list, callback)
			end
		end)
